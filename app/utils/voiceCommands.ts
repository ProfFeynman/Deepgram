...
import { processTranscriptionWithLLM } from './llmProcessor';
 ...
 * Uses regex patterns first, then falls back to LLM if no match is found
 * @param transcript The transcribed text to analyze
 * @param useLLM Whether to use LLM if regex fails
 * @returns An object with the recognized command and any additional data
 */
export const analyzeTranscript = async (transcript: string, useLLM = true): Promise<{ 
  command: string | null; 
  data: any;
  actionTaken: boolean;
  source?: 'regex' | 'llm';
  ...
  // First try with regex patterns (faster and no API call needed)
  const regexResult = checkRegexPatterns(transcript);
  
  if (regexResult.actionTaken) {
    return { ...regexResult, source: 'regex' };
  }
  
  // If regex didn't find a match and LLM is enabled, try with LLM
  if (useLLM) {
    try {
      const llmResult = await processTranscriptionWithLLM(transcript);
      
      if (llmResult.commandDetected && llmResult.functionName) {
        // Handle the function call from LLM
        const { functionName, args } = llmResult;
        
        switch (functionName) {
          case 'openWebsite':
            if (args.website) {
              const website = args.website.toLowerCase();
              let url = WEBSITE_URLS[website];
              
              // If custom URL provided or website not in predefined list
              if (args.customUrl || !url) {
                url = args.customUrl;
                // Add to website URLs if not already there
                if (args.website && args.customUrl) {
                  addCustomWebsite(args.website, args.customUrl);
                }
              }
          ...
          case 'browserAction':
            if (args.action) {
              executeNavigationCommand(args.action);
              return {
                command: args.action,
                data: null,
                actionTaken: true,
  ...
  // No command found with either method
  return { command: null, data: null, actionTaken: false };
};

/**
 * Check transcript against regex patterns
 * @param transcript The transcribed text to analyze
 * @returns Result object with command information
 */
const checkRegexPatterns = (transcript: string) => {
 ...
 * @param customUrl Optional custom URL to use instead of predefined ones
 */
export const openWebsite = (website: string, customUrl?: string): void => {
  const url = customUrl || WEBSITE_URLS[website];
  if (url) {
